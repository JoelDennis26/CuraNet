...
def get_recent_doctors(db: Session, limit: int = 5):
    """
    Fetch the most recent doctors added to the database.
    Orders by ID in descending order (assuming higher ID = more recent)
    Returns at most 'limit' number of doctors (default 5)
    """
    try:
        doctors = (
            db.query(models.Doctor).order_by(desc(models.Doctor.id)).limit(limit).all()
        )
        list = []
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving recent doctors: {str(e)}"
        )
